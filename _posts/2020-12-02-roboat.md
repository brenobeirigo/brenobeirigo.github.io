...
| Use case | Actors | Why? | What |
|:-:|:-:|-|-|
| [MoD](#passenger)  | AVs, passengers | On-street transportation may take longer for certain ODs. | AVs pickup and delivery passengers on demand. Embarking can occur at the closest canal using [flexible edge interfaces](#interface).  |
| [MoD with transfers](#transfer) | AVs, passengers, TPs | Fixed jetties offer low flexibility for servicing users. | System identifies candidate routes and AVs transfer passengers dynamically, (i) over water, (ii) at fixed transfer points, and (iii) at [temporary platforms (TP)](#tempinfra).
| [Routing touristic tours](#tourist) |  AVs, tourists, TAs | Boat tours follow a predetermined route. | Personalized boat tours are created on-demand. Tourists can hop-on anywhere and hop-off at the *touristic attractions* (TAs). |
| [Mooring private boats](#mooring) | AVs, BOs, private boats| *Boat owners* (BOs) must travel to retrieve their boats (sometimes moored outside city). | AVs tug private boats to owners' locations on-demand. |
| [Top-deck swapping](#modular) | OPs, AVs, cargo | For single-purpose AV fleets, low demand = low productivity. | Modular AVs are outfitted at *outfitting points* (OPs) to handle heterogeneous demands (e.g., different top-decks for passengers, waste, logistics, etc.). |
{:.table-striped .table-small}

# Actor description

Actors are entities (individuals, external systems)  that interact with the fleet management system entailed by each use case.

...
In TRiLOGy, we will have two main use cases: logistics and mobility.  
...
|-|-|-|-|
| [Waste collection](#waste_fixed) | residents, businesses | Heavy trucks damage quay walls & increase traffic, curbside garbage is visual pollution, poor waste separation. | Roboats pick up and deliver containers. Containers are *emptied at static location(s)*. |
| [Waste collection with waste barges](#waste_floating) | residents, businesses | Heavy trucks damage quay walls & increase traffic, curbside garbage is visual pollution, poor waste separation. | Roboats pick up and deliver containers. Containers are *emptied at (larger) waste vessels. Larger ships are emptied at a set of static waste facilities*. |
| [Goods delivery with floating warehouses](#logistics) | businesses | Heavy trucks damage quay walls & increase traffic, logistical operations increases on-street traffic. | *floating warehouses* (FW) hubs (tugged by Roboats) are loaded at warehouse(s) (outside the city). *Cargo* is delivered at city hubs using a ship equipped with a crane.  *Cargo* goes on to fulfill *businesses* (e.g., HORECAs, supermarkets, etc.) through electric trolleys.|
| [Construction material transportation with floating warehouses](#logistics) | businesses | Heavy trucks damage quay walls & increase traffic, logistical operations increases on-street traffic, infrastructure projects lead to poorer accessibility. | Ships load at warehouse(s) (outside the city) and unload at city hubs from where *building materials* go on to fulfill *construction sites* through electric trolleys. Floating storage hubs (or, FWs) are used temporarily to hold cargo.|
| [Parcel delivery with floating warehoses and drones](#drones) | senders, receivers | Parcel delivery increases on-street traffic (curbside use by vans).| Roboats tug *floating warehouses* (FW) with goods delivered by drones. |
...
| Use case | Actors | Why? | What |
|-|-|-|-|
| [MoD](#passenger)  | passengers | On-street transportation may take longer for certain ODs. | Roboats pickup and delivery passengers on demand. Embarking can occur at the closest canal using [flexible edge interfaces](#interface).  |
| [MoD with transfers](#transfer) | passengers | Fixed jetties offer low flexibility for servicing users. | System identifies candidate routes and roboats transfer passengers dynamically, (i) over water, (ii) at fixed transfer points, and (iii) at [temporary platforms](#tempinfra).
| [Routing touristic tours](#tourist) | tourists | Boat tours and museum visits follow a predetermined route. | Personalized boat tours are created on-demand. Tourists can hop-on anywhere and hop-off at the sightseeings. |
...
| [Top-deck swapping](#modular) | miscellaneous | For single-purpose Roboat fleets, low demand = low productivity. | Modular Roboats are outfitted at service points to handle heterogeneous demands (e.g., different top-decks for passengers, waste, logistics, etc.). |
{:.table-striped .table-small}
...
|TT|Trash thrower|City resident, business employee, construction worker, etc.|
|WB|Waste barge|Engineless barge tugged by AVs throughout the waterways.|
|WMF|Waste management facility|The end point of waste (e.g., incineration plant, transfer station, landfill).|
|FW|Floating warehouse|Loaded outside the city with cargo (parcels, construction material, etc.) and tugged by AVs to the city center.|
|CH|City hub|A waterfront location where cargo can be handled.|
| |Drone|Deliver parcels from FWs to users.|
| |Warehouse|Warehouse outside the city center where cargo is loaded/unloaded by FW.|
{:.table-striped .table-small}
## Mobility-on-demand
...
|-|-|-|
|AV|Autonomous vessel|Modular autonomous vessel whose hull can be outfitted with different top-decks (e.g., Roboat).|
||Service point|Where AVs are outfitted with different top-docks.|
||Passenger|Commuter, tourist, etc.|
||Boat owner|Private boat owner who wants to retrieve their boat moored elsewhere.|
{:.table-striped .table-small}
...
* Can a single Roboat service several containers at once (train)?
...
***
## Logistics - Waste collection + waste barges <a name="waste_floating"></a>

Roboats move filled containers to *waste barges* (WBs), where the waste is unloaded (two-layer routing).
In this setting, Roboats do not need to travel to *waste management facilities* (WMF) to unload containers, taking waste immediately to the closest WB.
...
* How many WBs and Roboats?
* What is the best route for Roboats and WBs?
...
*Floating warehouses* (FWs) are loaded outside the city and tugged by Roboats to the city center. Then:
...
* How to manage the battery usage of Roboats and drones (charging stations, charging barges)?
...
Roboats could be used as an alternative mode of transport to improve Amsterdam's museum experience by combining museum visits and boat tours.
...
Roboat units can join together to create temporary bridges, alleviating congestion on Amsterdam's centuries-old bridges and canal-side streets. As an autonomous system, Roboat can respond in real-time to the ebb and flow of rush hour traffic.
